function [ClustStruct,ClustData] = symmetry_cluster1(ImageMatrix, epsilon, minPts, ptID, s, percent) %CHANGED PERCENT to PERCENT_VAL

%With percent threshold
I = getMatrixOutliers(ImageMatrix);   % Remove Outliers
I_adj = I(find(I>0));
percent2 = percent / 100;
I_sort = sort(I_adj,'descend');       % Arrange Image Hist in Order Low -> High
percent_ind = round(percent2 * numel(I_sort));   % Find the index number for the User Input Percentage
percent_val = I_sort(percent_ind);        % Find Intensity for the Percentage Number
