...
clc;
% Read Image
sa=imread_irfanview('1799 - P4.tif');
...
%Nada: The method above is removing pixe ls of interest in the breast
...
boundary_right = bounds{2}; %gives the properties of the second boundary, which is the right breast
%  figure
%  plot(boundary(:,2), boundary(:,1),'r'); 

row_right=boundary_right(:,1); % rows of right breast boundary

col_right=boundary_right(:,2); % columns of right breast boundary

boundary_left = bounds{3};  %Nada: Should be the left breast boundry

row_left=boundary_left(:,1); % rows of left breast boundary

col_left=boundary_left(:,2); % columns of left breast boundary


 %% Perform Segmentation

 
% [H,theta,rho] = hough(bwImgs); %Zainab: find hough transform
% 
% figure;
% imshow(sa);
% imshow(imadjust(mat2gray(H)),'XData',theta,'YData',rho);

% circles = imfindcircles(bwImgs,[50 100]); %Zainab: hough using circle detection

params.minMajorAxis = 300;
params.maxMajorAxis = 400;
params.rotation = 90;
params.rotationSpan = 60;
params.randomize = 0;
params.numBest = 8;

bestFits = ellipseDetection(edge, params);
imshow(sa);
hold on;
ellipse(bestFits(:,3),bestFits(:,4),bestFits(:,5)*pi/180,bestFits(:,1),bestFits(:,2),'r');

% params_l.minMajorAxis = 200;
% params_l.maxMajorAxis = 300;
% 
% bestFits_l = ellipseDetection(bwImgs, params_l);
% ellipse(bestFits_l(:,3),bestFits_l(:,4),bestFits_l(:,5)*pi/180,bestFits_l(:,1),bestFits_l(:,2),'r');
