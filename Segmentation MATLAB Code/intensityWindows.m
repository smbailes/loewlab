    ...
    for i = 1:length(gettx)
        if getty(i,1)>midy(1,1)
            ...
            if (gettx(i,1)>Xright && getty(i,1)>Yright) || (gettx(i,1)<Xright && getty(i,1)<Yright)
                ...
                for k = 1:len
                    linepix1(i,k,1) = intwind(i,2) + xdiff(i,k);
                    linepix1(i,k,2) = intwind(i,1) - ydiff(i,k);
                    linepix2(i,k,1) = intwind(i,2) - xdiff(i,k);
                    linepix2(i,k,2) = intwind(i,1) + ydiff(i,k);
                end
            end
        end
    end 
