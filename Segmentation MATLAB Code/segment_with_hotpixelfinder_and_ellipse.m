...
% UPDATE:
% Last Modified by Zainab Mahmood: 06/17/2017 at 3:14 PM
%     - Merged hotpixel with canny code and ellipse code
%     - Possible approach for detecting overlap between all 3 codes
%         1. Detect which ellipses have any pixels overlapping with
%         hotpixelfinder and keep those while deleting other ellipses
%         2. Then go pixel by pixel through the ellipses that are left and
%         check if those pixels overlap with hot pixels, remove if not
%         overlapping
%     - Managed to pixelate ellipses by creating matrix and plot it on
%     image, then delete pixels that didn't overlap with hot pixels. But
%     this removed a majority of the ellipse
...
for n=1:length(radii)
    h{n} = viscircles(centers,radii,'LineWidth',1);
end
...
clear n;
figure, imshow(boundaries_hotpixel), title('Boundaries with HotPixelFinder')
...
%% Part 4.5: Find ellipses that overlap with hotpixel finder and keep those, remove others
% e1=qr{1};
% e2=qr{2};
% xe1=e1.XData;
% ye1=e1.YData;
...
xe1 = zeros(length(ql),[]);
ye1 = zeros(length(ql),[]);
ellipse_matrix = zeros(480,640);        %create matrix of zeros as big as the image
...
for a = 1:length(ql)                %a,b,n,m are just used as counters in the for loops - delete at end of section
    ...
    for b = 1:length(e1.XData)              %get x and y data from cell array of ellipses and round so we can use them as indices
        ...
        if xe1(a,b)<0 || xe1(a,b)==0            %some ellipse indices were negative (off screen) so this is to correct for that
            xe1(a,b)=1;
        end
        if ye1(a,b)<0 || ye1(a,b)==0
            ye1(a,b)=1;
        end
    end
    for d = 1:length(xe1)
...
for a = 1:length(qr)                %a,b,n,m are just used as counters in the for loops - delete at end of section
    ...
    for b = 1:length(e2.XData)
        ...
        if xe2(a,b)<0 || xe2(a,b)==0
            xe2(a,b)=1;
        end
        if ye2(a,b)<0 || ye2(a,b)==0
            ye2(a,b)=1;
        end
    end
    for d = 1:length(xe2)
...
ellipse_matrix_thick = strel('line',5,45);      %morphological operations to close gaps between points, making a line
ellipse_matrix_thick = imclose(ellipse_matrix,ellipse_matrix_thick);
canny_ellipse = edge(ellipse_matrix_thick,'canny'); %get canny edges

%get first column of canny_ellipse edges so line is only 1 pixel thick
canny_ellipse_boundary = zeros(480,640);
[ro,co] = size(canny_ellipse_boundary);
for z = 1:co
    for zz = 1:ro
        if canny_ellipse(zz,z)==0
            continue;
        else
            canny_ellipse_boundary(zz,z)=1;
            break;
        end
    end
end

c_e_boundary_thick = strel('line',5,45);
c_e_boundary_thick = imclose(canny_ellipse_boundary,c_e_boundary_thick);
% c_e2 = edge(c_e_boundary_thick,'canny');
% 
% c_e2_long = bwareaopen(c_e2,20);

sobel = edge(I,'sobel');        %detect sobel edges

%get bottom half of ellipse (ellipse_matrix)


figure;
imshow(I,[]);           %image on which we're drawing ellipses
title('Image with hotpixel overlapping with ellipse (lets hope!)');
%plot only the ellipses that were kept after the for loop onto original
%image
magenta = cat(3, ones(size(imgOG)), zeros(size(imgOG)), ones(size(imgOG))); %magenta has RGB value of 1 0 1
hold on 
h3 = imshow(magenta); 
set(h3, 'AlphaData', c_e_boundary_thick);    %change the name here if you want to change what you're plotting on the og image
set(gca,'LineWidth',4);
h4 = imshow(magenta);
set(h4, 'AlphaData', sobel);
