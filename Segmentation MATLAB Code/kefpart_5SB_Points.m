%Small Breast Points
% Point system code.
% The idea is to look pixel by pixel to determine which should be included.
% This version has heavier weight on the circles
...
for aa = 1:img_y
    for bb = 1:img_x
        ...
        if lowerellipse(aa,bb)==2
            final_im(aa,bb)=final_im(aa,bb)+1;
        end
        if canny_bounded(aa,bb)==1
            final_im(aa,bb)=final_im(aa,bb)+3;
        end
        if circs(aa,bb)==1
            final_im(aa,bb)=final_im(aa,bb)+3;
        end
...
figure, imshow(I,[]), title('Small Breast Pre-Cleaning')
