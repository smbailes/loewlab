...
%% Using Quartile Method - ONLY REMOVING POS OUTLIERS (> 1.5*IQR)
    [nr, nc] = size(matrix);
    A = reshape(matrix,[1, numel(matrix)]); %make matrix into single vector
    B = sort(A);
    
    ind75 = round(0.75*numel(B));
    ind25 = round(0.25*numel(B));
    ...
    for i = 1:(length(x_pos))
        if (x_pos(i) == 1)&&(y_pos(i) == 1) %TOP LEFT CORNER
            newmat(y_pos(i),x_pos(i)) = mean([matrix(y_pos(i),x_pos(i)+1)  %Replace with Average of surrouding pixels
        ...
        elseif (x_pos(i) == 1) && (y_pos(i) == nr) %BOTTOM LEFT CORNER
            newmat(y_pos(i),x_pos(i)) = mean([matrix(y_pos(i)-1,x_pos(i))  %Replace with Average of surrouding pixels
                                        matrix(y_pos(i)-1,x_pos(i)+1) 
                                        matrix(y_pos(i),x_pos(i)+1)]);
        
        elseif (x_pos(i) == nc) && (y_pos(i) == nr) %BOTTOM RIGHT CORNER
            newmat(y_pos(i),x_pos(i)) = mean([matrix(y_pos(i)-1,x_pos(i)-1)  %Replace with Average of surrouding pixels
                                        matrix(y_pos(i)-1,x_pos(i)) 
                                        matrix(y_pos(i),x_pos(i)-1)]);
