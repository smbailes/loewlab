...
alllowsquarechange{v} = lowsquarechange;
...
bar(c,bardata)
title('Average Rate of change')
%% Calculate percent change of warm regions
answer = questdlg('Which side is the warmest region on?','WarmregionSide','Left','Right','Left') % used to get total data accross all
if isequal(answer,'Left') == 1
    perchangebreast{v} = ((totLbreastchange - totwarmchange)/totLbreastchange)*100
elseif isequal(answer,'Right') ==1
    perchangebreast{v} = ((totRbreastchange - totwarmchange)/totRbreastchange)*100
end
perchangecorr{v} = ((totcorrwarmchange - totwarmchange)/totcorrwarmchange)*100
else
